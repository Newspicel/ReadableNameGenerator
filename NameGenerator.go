...
var (
    starts = []string{"a", "b", "c", "d", "e", "f", "g", "i", "j", "k", "l", "m", "n", "o", "r", "p", "s", "t", "u"}
    speakable = map[string][]string{
...
func Generate() string {
    ...
    char := starts[rand.Intn(len(starts))]

    name := char

    for i := 0; i < length; i++ {
        char = speakable[char][rand.Intn(len(speakable[char]))]
        name += char
    }
    return name
}
